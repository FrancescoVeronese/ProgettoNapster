        ...
        if(scelta==1):
            ...
            if(scelta==2):
                ...
                if(scelta==3):
                    ...
                    s.send(response.encode())
                    risultato=s.recv(1024).decode()
                    numfile=risultato[4:7]
                    print("Il server ha trovato ha i seguenti file...\n")
                    for n in numfile:
                        s.recv(1024).decode()
                        filemd5=risultato[0:32]
                        filename=risultato[32:132]
                        copies=risultato[132:135]
                        ippeer=risultato[135:150]
                        portpeer=risultato[150:155]

                        peer=(filemd5,filename,copies,ippeer,portpeer)
                        print("%s\n,%s\n,%s\n,%s:%s",peer)
                    ...
                    if(scelta==4):
                        ...
                        download(host, porta, filecode)
                        response = servdownload(sessionID)
                        s.send(response.encode()) # informo il server del download
